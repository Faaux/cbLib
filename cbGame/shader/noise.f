...
float smoothThing(vec3 x, int octave, float persistence, float scale, float low, float high)
{
	float maxAmp = 0;
	float amp = 1;
	float freq = scale;
...
void main()
{
	...
	vec3 tempColor = mix(white, red, smoothstep(0.1, 0.9, noise));
